...
export async function onRequest(context) {
  ...
  try {
    ...
    const siteUrl = (env.SITE_URL || 'https://celsheadspace.pages.dev').replace(/\/$/, '');
  ...
  const tryProviders = [
    async () => {
      ...
      const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${env.OPENROUTER_API_KEY}`, 'Content-Type': 'application/json', 'HTTP-Referer': env.SITE_URL || 'https://celsheadspace.pages.dev' },
