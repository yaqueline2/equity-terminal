...
## 4. PWA / offline
- `manifest.json` (display standalone, icons), `sw.js` (cache **`cel-headspace-v8`**, network-first for navigation so edits show, cache-first for stable media), `icons/` (now the pink-flower app icon). The mutable quote archive loads through `assets/douyin/quotes.js?v=archive-2` and `_headers` requires browser revalidation; never restore a blanket immutable `/assets/*` rule because it makes new article metadata stale on Cloudflare.
...
- Cloudflare quote archive cache repair: production diagnosis showed the current HTML and MP3 were deployed while the browser retained an older `quotes.js` under the blanket seven-day immutable asset policy. The page and service worker now request the versioned `quotes.js?v=archive-2`; `_headers` makes quote metadata revalidate while keeping stable images/audio immutable. Service-worker cache v8 forces the installed app onto the corrected archive URL.
