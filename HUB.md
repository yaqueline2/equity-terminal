...
## 4. PWA / offline
- `manifest.json` (display standalone, icons), `sw.js` (cache **`cel-headspace-v7`**, network-first for navigation so edits show, cache-first for assets incl. `webp`), `icons/` (now the pink-flower app icon).
