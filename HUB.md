...
## 4. PWA / offline
- `manifest.json` (display standalone, icons), `sw.js` (cache **`cel-headspace-v9`**, network-first for navigation so edits show, cache-first for stable media), `icons/` (now the pink-flower app icon). The mutable quote archive loads through `assets/douyin/quotes-archive.js` and `_headers` requires browser revalidation; never restore a blanket immutable `/assets/*` rule because it makes new article metadata stale on Cloudflare.
...
- Cloudflare quote archive cache repair: production diagnosis showed the current HTML and MP3 were deployed while the browser retained an older `quotes.js` under the blanket seven-day immutable asset policy. The mutable bundle now has a fresh `quotes-archive.js` path, `_headers` makes it revalidate while keeping stable images/audio immutable, and service-worker cache v9 forces installed copies onto that new path.
