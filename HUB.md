...
Keychain item; it must never enter HTML, Git, logs, memory, or chat. Finn first
uses `acc-finance-read-finn` to refresh the live snapshot, writes and validates
the ignored local pending snapshot, then uses `acc-finance-sync-finn`. Success
promotes the update locally and clears pending state. Finn never asks Celine for
an export, import, manual entry, connection step, or another approval. UI,
schema, and feature changes still use the reviewed Headspace publication
workflow. The working contract is `../skills/finn_finance_handoff.md`.
...
- Direct Finn finance access (23 Jul 2026): retired export/import completely. Finn reads the live private Cloudflare snapshot, validates confirmed changes, and writes them back through bounded Keychain-backed commands without asking Celine to transfer files or enter supported data manually. Private values remain absent from public HTML and Git. New finance fields, stores, and UI are still implemented as versioned Headspace code changes.
